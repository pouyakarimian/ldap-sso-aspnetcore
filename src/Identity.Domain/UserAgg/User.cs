...
using Identity.Domain.UserAgg.Enums;
using Identity.Domain.UserAgg.Services;
    ...
    public class User : AggregateRoot
    {
        ...
        public void Guard(string phoneNumber, string email, IUserDomainService userDomainService)
        {
            ...
            if (phoneNumber.Length != 11)
                throw new InvalidDomainDataException("Invalid mobile number");

            if (!string.IsNullOrWhiteSpace(email))
                if (email.IsValidEmail() == false)
                    throw new InvalidDomainDataException("Invalid email");

            if (phoneNumber != PhoneNumber)
                if (userDomainService.PhoneNumberIsExist(phoneNumber))
                    throw new InvalidDomainDataException("The mobile number is duplicate");

            if (email != Email)
                if (userDomainService.IsEmailExist(email))
                    throw new InvalidDomainDataException("Duplicate email");
        }
    }
}